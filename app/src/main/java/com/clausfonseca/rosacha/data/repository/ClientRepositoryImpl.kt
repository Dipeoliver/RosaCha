    ...
    override fun getClients(dbClient: String, clientList: MutableList<ClientModel>): Flow<Resource<MutableList<ClientModel>>> =
        callbackFlow {
            try {
                ...
                trySend(Resource.Loading())
                fireStore.collection(dbClient).orderBy("name").limit(10).get().addOnSuccessListener { querySnapshot ->
...
//                            val key = result.id // pegar o nome  da pasta do documento
                        ...
                        nextQuery =
                            fireStore.collection(dbClient).orderBy("name").startAfter(lastResult).limit(10)
                    } else {
